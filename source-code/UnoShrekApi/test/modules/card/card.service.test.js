...
describe("GameService", () => {
  ...
  describe("create", () => {
    ...
    test("should throw when creating a card for a non-existing game", async () => {
      ...
      await expect(cardService.create(validCardData)).rejects.toThrow(
        NotFoundError,
      );
    });
  });
  ...
  describe("read", () => {
    ...
    test("should return a card by id", async () => {
      jest
        .spyOn(CardRepository.prototype, "getById")
        .mockResolvedValue(existingCard);
    ...
    test("should throw when retrieving a non-existing card", async () => {
      ...
      await expect(cardService.getById("missing_card")).rejects.toThrow(
        NotFoundError,
      );
    });
  ...
  describe("update", () => {
    ...
    test("should update a card successfully", async () => {
      ...
      mockGameService.getById.mockResolvedValue({ _id: validCardData.gameId });
      jest
        .spyOn(CardRepository.prototype, "update")
        .mockResolvedValue({ ...existingCard, color: "blue" });

      const result = await cardService.update(existingCard._id, {
        color: "blue",
      });
    ...
    test("should throw when updating a card to a non-existing game", async () => {
      ...
      await expect(
        cardService.update(existingCard._id, {
  ...
  describe("delete", () => {
    ...
    test("should delete a card successfully", async () => {
      jest
        ...
        .mockResolvedValue(existingCard);
      jest
        .spyOn(CardRepository.prototype, "deleteById")
    ...
    test("should throw when deleting a non-existing card", async () => {
      ...
      await expect(cardService.deleteById("missing_card")).rejects.toThrow(
        NotFoundError,
      );
    });
  });

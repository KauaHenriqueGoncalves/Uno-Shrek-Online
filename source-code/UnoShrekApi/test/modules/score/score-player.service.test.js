...
describe("ScorePlayerService", () => {
  ...
  describe("read", () => {
    ...
    test("should return a score entry by id", async () => {
      jest
        .spyOn(ScorePlayerRepository.prototype, "getById")
        .mockResolvedValue(existingScore);
    ...
    test("should throw when retrieving a non-existing score entry", async () => {
      ...
      await expect(scorePlayerService.getById("missing_score")).rejects.toThrow(
        NotFoundError,
      );
    });
    ...
    test("should return score details including player and game", async () => {
      jest
        .spyOn(ScorePlayerRepository.prototype, "getById")
        .mockResolvedValue(existingScore);
      mockPlayerService.getById.mockResolvedValue({
        _id: validScoreData.playerId,
        username: "davi",
      });
      mockGameService.getById.mockResolvedValue({
        _id: validScoreData.gameId,
        title: "Sala 1",
      });
  ...
  describe("create", () => {
    ...
    test("should create a score entry successfully", async () => {
      ...
      jest
        .spyOn(ScorePlayerRepository.prototype, "create")
        .mockResolvedValue(existingScore);

      const result = await scorePlayerService.create(validScoreData);

      expect(mockPlayerService.getById).toHaveBeenCalledWith(
        validScoreData.playerId,
      );
      expect(mockGameService.getById).toHaveBeenCalledWith(
        validScoreData.gameId,
      );
      expect(result).toEqual(existingScore);
    });
    ...
    test("should throw when creating a score for a non-existing player", async () => {
      ...
      await expect(scorePlayerService.create(validScoreData)).rejects.toThrow(
        NotFoundError,
      );
    });
  });
  ...
  describe("update", () => {
    ...
    test("should update a score entry successfully", async () => {
      ...
      mockGameService.getById.mockResolvedValue({ _id: validScoreData.gameId });
      jest
        .spyOn(ScorePlayerRepository.prototype, "update")
        .mockResolvedValue({ ...existingScore, score: 25 });

      const result = await scorePlayerService.update(existingScore._id, {
        score: 25,
      });
  ...
  describe("delete", () => {
    ...
    test("should delete a score entry successfully", async () => {
      jest
        ...
        .mockResolvedValue(existingScore);
      jest
        .spyOn(ScorePlayerRepository.prototype, "deleteById")
    ...
    test("should throw when deleting a non-existing score entry", async () => {
      ...
      await expect(
        scorePlayerService.deleteById("missing_score"),

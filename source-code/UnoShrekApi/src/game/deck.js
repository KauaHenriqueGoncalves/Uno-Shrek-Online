...
/**
 * Check whether `card` can legally be played on top of `topCard`.
 * Wild cards are always valid (color is chosen at play time).
 * Normal cards match by color or type/value.
 * `activeColor` is the currently declared color (relevant after a wild).
 */
export function isValidPlay(card, topCard, activeColor) {
    ...
    const matchType = card.type !== "number" ? card.type === topCard.type
...
/**
 * Apply the effect of a played card and return an update object.
 * The caller (GameEngine) applies the update to the game state.
 */
